...
Run the documented Podman build/run block in `USER_REQUIREMENTS.md`, return its complete output, and leave the container running.
...
1. User builds and starts the OCI container with Podman and returns the requested output.
2. Codex verifies Tesseract and application behavior inside the running container, benchmarks live OCR, runs all automated gates, and updates README evidence.
